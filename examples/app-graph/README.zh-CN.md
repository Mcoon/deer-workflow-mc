...
- 后续同 App version、同 device profile 调用：严格重启后复用已验证的完整 recipe
  （包括必要的 swipe 和 Binding），省略逐步 UI dump，只在最后做一次 Ground/Oracle；
- fast 最终验证失败：Task/Operator/Binding 自动降回 `guarded`，重新严格重启并安全执行。

成功沉淀的是完整路径，不是孤立点击坐标。例如“打开帮助与反馈”的 recipe 是
`打开设置 → 设置列表 swipe → 点击帮助与反馈`。
